...
var dorita980 = function localV2 (user, password, host, customOptions = {}) {
  if (!user) throw new Error('robotID is required.');
  if (!password) throw new Error('password is required.');
  if (!host) throw new Error('host is required.');

  /* Backwards compatibility for when the final argument was the optional emitIntervalTime value */
  if (typeof customOptions === 'number') {
    customOptions = {
      emitIntervalTime: customOptions,
    };
  }

  const posibleCap = ['pose', 'ota', 'multiPass', 'carpetBoost', 'pp', 'binFullDetect', 'langOta', 'maps', 'edge', 'eco', 'svcConf'];
  const emitIntervalTime = customOptions.emitIntervalTime || 800;
  var robotState = {};
  var cap = null;
  var missionInterval;
  ...
  var options = {
    ...
    protocolId: 'MQTT',
    protocolVersion: 4,
    ciphers: process.env.ROBOT_CIPHERS || customOptions.ciphers || 'AES128-SHA256',
    clean: false,
    username: user,
    password: password,
  ...
  client.on('error', function () {
    /* Ignore. Don't throw as it can crash the whole app. */
  });
