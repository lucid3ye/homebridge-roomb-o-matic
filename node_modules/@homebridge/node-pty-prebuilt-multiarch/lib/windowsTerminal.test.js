        ...
        var interval = setInterval(function () {
            psList({ all: true }).then(function (ps) {
                ...
                if (tries * intervalMs >= timeoutMs) {
                    clearInterval(interval);
                    var processListing = pids.map(function (k) { return "".concat(k, ": ").concat(desiredState[k]); }).join('\n');
                    assert.fail("Bad process state, expected:\n".concat(processListing));
                    resolve();
                }
            });
        }, intervalMs);
    ...
    return new Promise(function (resolve) {
        ...
        var interval = setInterval(function () {
            psList({ all: true }).then(function (ps) {
                ...
                if (tries * intervalMs >= timeoutMs) {
                    clearInterval(interval);
                    assert.fail("Bad process state, expected: ".concat(size, ", actual: ").concat(list.length));
                }
            });
        }, intervalMs);
    });
...
if (process.platform === 'win32') {
    [true, false].forEach(function (useConpty) {
        describe("WindowsTerminal (useConpty = ".concat(useConpty, ")"), function () {
            ...
            describe('Args as CommandLine', function () {
                it('should not fail running a file containing a space in the path', function (done) {
                    ...
                    var data = fs.readFileSync("".concat(process.env.windir, "\\System32\\cmd.exe"));
